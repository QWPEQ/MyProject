    ...
    for (int index = 0; index < fd_count; ++ index)
    {
      ...
      else if (epoll_event_list_[index].events & EPOLLIN)
      {
        ...
        if (read_size > 0)
        {
          ...
          {
            buff_[read_size] = '\0';
            if (NULL != handler_ && endpoint_it != sock_endpoint_map_.end())
...
DWORD WINAPI SocketAccept::ServerWorkThread(LPVOID CompletionPortID)
{
  ...
  while (true)
  {
    ...
    if (NULL == pIoData || NULL == pHandleData)
      continue;
    ...
    if (bytesTransferred == 0 || get_queue_error)
    {
      ...
      if (NULL != pIoData->accept_ptr)
      {
        auto endpoint_it = pIoData->accept_ptr->sock_endpoint_map_.find(pHandleData->socket);

        if (NULL != pIoData->accept_ptr->handler_ && endpoint_it != pIoData->accept_ptr->sock_endpoint_map_.end())
    ...
    if (pIoData->type == IOCPHandleType::SEND)
    {
      if (all_bytes_transferred)
      {
        if (NULL != pIoData->accept_ptr)
        {
          LPPER_IO_OPERATION_DATA new_pio_data_ptr = NULL; // get next queue send data
          while (NULL == new_pio_data_ptr && !pIoData->accept_ptr->send_queue_map_[pHandleData->socket].empty())
          {
