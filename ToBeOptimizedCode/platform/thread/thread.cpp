...
Thread::Thread() : thread_id_(0), cur_task_(NULL), running_(false)
{
}
...
ThreadReturn Thread::StartRoutine(void *param)
{
	...
	if (NULL == thread || NULL == thread->cur_task_)
		return 0;

	thread->cur_task_->Run();

	delete thread->cur_task_;
	thread->cur_task_ = NULL;
	thread->Detach();
...
Thread::Thread() : thread_id_(0), cur_task_(NULL), running_(false), thread_handle_(NULL)
{
}
...
bool Thread::Run(IThreadTask *func, unsigned int stack_size)
{
  ...
  cur_task_ = func;
  thread_handle_ = CreateThread(NULL, stack_size, &Thread::StartRoutine, this, 0, &thread_id_);
  if (NULL == thread_handle_)
    return false;
