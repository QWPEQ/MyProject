...
void SerializeNetworkManager::OnRecvPackage(NetworkID net_id, char *data, int length)
{
  ...
  for (std::set<ISerializeNetworkHandler *>::iterator handler_it = serialize_handler_list_.begin(); handler_it != serialize_handler_list_.end(); ++ handler_it)
    //for (auto handler_ptr : serialize_handler_list_)
    (*handler_it)->OnRecv(net_id, serialize_data);
