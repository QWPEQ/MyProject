...
{
  for (std::vector<char*>::iterator buff_it = real_buffer_ptrs_.begin(); buff_it != real_buffer_ptrs_.end(); ++ buff_it)
...
MemoryPoolManager::~MemoryPoolManager()
{
  for (std::map<int, MemoryPool*>::iterator pool_it = memory_size_to_pool_map_.begin(); pool_it != memory_size_to_pool_map_.end(); ++ pool_it)
